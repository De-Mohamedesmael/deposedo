...
curl_close ($ch);
    ...
    public function clientLogin(Request $request)
    {
        ...
        if (Auth::guard('web')->attempt(['email' => $request->email, 'password' => $request->password], $request->get('remember'))) {
          ...
          // Alert::error('error', __("site.error in email or password"));
          // dd( __("site.error in email or password"));
        return back()->with('error_login', __("site.error in email or password"))->withInput($request->only('email', 'remember'));
    }
    ...
    protected function createClient(Request $request)
    {
        ...
        $rules = array(
            ...
            'phone'=>'required|Filled|numeric|unique:users,phone',
