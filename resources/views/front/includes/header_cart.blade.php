 <a href="{{route("cart.show")}}" class="nav-link   border-right"  >
   <i class="fas fa-shopping-bag" ></i> @lang('site.cart') <b>
  ...
  @foreach($details as  $detail)
      ...
      <div class="row  mr-0 border-bottom">
        ...
        <div class="col-9 ">
            ...
            <h6 class="">
              @if($product->if_sale)
                <b>{{get_price_helper($product->sale_price)}}</b>
                <span class="regular_price">{{get_price_helper($product->regular_price)}} </span>
              @else
...
<script>
  $('.price_cart_total').text("{{get_price_helper($price_c)}}");
</script>
@else
